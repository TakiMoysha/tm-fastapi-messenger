...
from app.router.urls import URL_ACCOUNT_SIGN_IN, URL_ACCOUNT_SIGN_UP
...
class TestAuth:
    async def test_should_create_user_account_by_sing_up(self, credentials: dict, client: TestClient):
        r = client.post(URL_ACCOUNT_SIGN_UP, json=credentials)
        logger.debug(r.headers)
        assert r.status_code == status.HTTP_201_CREATED, r.json()

    async def test_should_authenticate_user_by_sing_in(self, credentials: dict, client: TestClient):
